sglasso.fit <- function(Sv, mask, w, nrho, min_rho, nstep, algorithm, tol){
	...
	if(nrho > 1){
		if(algorithm == "ccd"){
			fit = .Fortran("sglasso_ccd_path", nSv = nSv, Sv = Sv, nTv = nTv, Tv_pkg = Tv_pkg, Tv_rw = Tv_rw, nv = nv, 
						   ...
						   Te_scn = Te_scn, Te_ptr_scn = Te_ptr_scn, nstep = nstep, tol = tol, rho = rho, 
						   nrho = nrho, min_rho = min_rho, grd = grd, th = th, w = w, df = df, n = n, conv = conv)
		} else {
		...
		if(algorithm == "ccd"){
			fit = .Fortran("sglasso_ccd_single", nSv = nSv, Sv = Sv, nTv = nTv, Tv_pkg = Tv_pkg, Tv_rw = Tv_rw, nv = nv, 
						   Tv_ptr = Tv_ptr, nTe = nTe, Te = Te, nTe_ptr = nTe_ptr, Te_ptr = Te_ptr,ne = ne, 
						   Te_scn = Te_scn, Te_ptr_scn = Te_ptr_scn, nstep = nstep, tol = tol, rho = min_rho, 
						   grd = grd, th = th, w = w, df = df, n = n, conv = conv)
		} else {
	...
	if(fit$conv!=0) warning("sglasso does not converge\n")
	fit
}
