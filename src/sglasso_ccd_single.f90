!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!						SUBROUTINE USED TO COMPUTE THE SGLASSO ESTIMATOR BY THE CCD ALGORITHM (SINGLE VERSION)
!
!	AUTHOR: Luigi Augugliaro
!	VERSION: 1.0-0
!	DATA: 23/04/2014
!
!	DESCRIPTION: the subroutine sglasso_ccd_single implements the cyclic coordinate descent algorithm proposed
!						Augugliaro et al., (2014) to fit a weighted l1-norm penalized RCON(V, E) model. 
!
!	ARGUMENTS: see arguments in sglasso_ccm_path.f90
subroutine sglasso_ccd_single(nSv,Sv,nTv,Tv_pkg,Tv_rw,nv,Tv_ptr,nTe,Te,nTe_ptr,Te_ptr,ne,Te_scn,Te_ptr_scn, &
	nstep,tol,rho,grd,th,w,df,n,conv)
	integer	:: nSv,nTv,Tv_pkg(nTv),Tv_rw(nTv),nv,Tv_ptr(nv+1),nTe, Te(nTe),nTe_ptr,Te_ptr(nTe_ptr),ne, &
		Te_scn(ne+1),Te_ptr_scn(ne+1),nstep,df,n,conv
	double precision	:: grd(nv+ne),Sv(nSv),tol,rho,th(nv+ne),w(ne)
	!internal variables
	logical	:: A(ne),flg
	integer	:: p,i,j,h,k,i0,i1,j0,j1,h0,h1,Te_scn_sz(ne), &
		Te_cl_ptr(ne+1),Te_cl_sz(ne),Te_cl(ne*(Tv_ptr(nv+1)-1)),max_df
	...
	do
			...
			do i = 1, ne
				if(A(i)) then
					...
					if(abs(Du).gt.tol) then
						...
						if(dsign(dble(1),th(nv + i) + dth).ne.Du_sgn) then
							dth = - th(nv + i)
							th(nv + i) = 0.
							A(i) = .false.
							df = df - 1
		...
		th_diff = sum((th(1:nv) - oth(1:nv))**2)
		if(th_diff.lt.tol) then
			do i = 1, ne
				if(A(i)) then
					th_diff = th_diff + (th(nv + i) - oth(nv + i))**2 
				end if
			end do
		end if
...
end subroutine sglasso_ccd_single
