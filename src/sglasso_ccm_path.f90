...
!
!		INPUT
!			nSv = length of the vector Sv;
!			Sv = nSv-dimensional vector storing the upper triangular of the empirical variance/covariance matrix;
!			nv = number of vertex colour classes;
!			nTv, Tv_pkg, Tv_rw, Tv_ptr = vectors used to handle the matrices D_n defining the structured concentration
!													matrix. The compressed column storage (ccs) is used in this implementation;
...
!			min_rho = last value of the sequence of tuning parameters used to compute the sglasso solution path. 
!							If the argument nrho = 1, then min_rho is the value used to compute the sglasso estimate. 
!							Default value is 1.0e-02;
!
!		INPUT/OUTPUT
!			w = ne-dimensional vector of the weights used to define the l1-norm. If not specified (default in
!					sglasso.R), in output the estimates of the variance of the elements of the score vector are
!					returned;
!
!		OUTPUT
!			grd = (nv + ne, nrho) dimensional matrix used to store the gradient vector; 
!			th = (nv + ne, nrho) dimensional matrix used to store the estimates of the parameter of the RCON(V, E) model;
!			df = nrho-dimensional vector used to store the number of estimated non-zero parameters;
!			n = number of iterations done by the ccm algorithm;
!			conv = integer used to code the convergence of the algorithm:
!					con = 0 => convergence of the algorithm has been achieved
!					con = 1 => maximum number of iterations has been achieved
!
subroutine sglasso_ccm_path(nSv,Sv,nTv,Tv_pkg,Tv_rw,nv,Tv_ptr,nTe,Te,nTe_ptr,Te_ptr,ne,Te_scn,Te_ptr_scn,nstep,tol, &
...
end subroutine sglasso_ccm_path
